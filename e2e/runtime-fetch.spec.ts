...
test.describe('desktop runtime routing guardrails', () => {
    ...
    const result = await page.evaluate(async () => {
      ...
      return {
        ...
        webHost: runtime.detectDesktopRuntime({
          ...
          userAgent: 'Mozilla/5.0',
          locationProtocol: 'https:',
          locationHost: 'worldmonitor.snakeg0d.com',
          locationOrigin: 'https://worldmonitor.snakeg0d.com',
        }),
      };
    });
  ...
  test('runtime fetch patch falls back to cloud for local failures', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL) => {
        ...
        if (url.includes('worldmonitor.snakeg0d.com/api/fred-data')) {
        ...
        if (url.includes('worldmonitor.snakeg0d.com/api/stablecoin-markets')) {
    ...
    expect(result.calls.some((url) => url.includes('worldmonitor.snakeg0d.com/api/fred-data'))).toBe(true);
    expect(result.calls.some((url) => url.includes('127.0.0.1:46123/api/stablecoin-markets'))).toBe(true);
    expect(result.calls.some((url) => url.includes('worldmonitor.snakeg0d.com/api/stablecoin-markets'))).toBe(true);
  });
  ...
  test('runtime fetch patch never sends local-only endpoints to cloud', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL) => {
        ...
        if (url.includes('worldmonitor.snakeg0d.com/api/local-env-update')) {
          return responseJson({ leaked: true }, 200);
        }
        if (url.includes('worldmonitor.snakeg0d.com/api/local-validate-secret')) {
          return responseJson({ leaked: true }, 200);
        }
    ...
    expect(result.calls.some((url) => url.includes('worldmonitor.snakeg0d.com/api/local-env-update'))).toBe(false);
    expect(result.calls.some((url) => url.includes('worldmonitor.snakeg0d.com/api/local-validate-secret'))).toBe(false);
  });
  ...
  test('update badge picks architecture-correct desktop download url', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      const previousTauri = globalWindow.__TAURI__;
      const releaseUrl = 'https://github.com/snakeg0d/worldmonitor/releases/latest';
    ...
    expect(result.macArm).toBe('https://worldmonitor.snakeg0d.com/api/download?platform=macos-arm64&variant=full');
    expect(result.windowsX64).toBe('https://worldmonitor.snakeg0d.com/api/download?platform=windows-exe&variant=full');
    expect(result.linuxFallback).toBe('https://github.com/snakeg0d/worldmonitor/releases/latest');
  });
  ...
  test('cloud fallback blocked without WorldMonitor API key', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      try {
        ...
        const cloudCalls = calls.filter(u => u.includes('worldmonitor.snakeg0d.com'));
  ...
  test('cloud fallback allowed with valid WorldMonitor API key', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.includes('worldmonitor.snakeg0d.com') && init?.headers) {
        ...
        if (url.includes('worldmonitor.snakeg0d.com/api/market/v1/test')) {
      ...
      try {
        ...
        return {
          ...
          cloudCalls: calls.filter(u => u.includes('worldmonitor.snakeg0d.com')).length,
